...
This may take a moment while the Shipyard images are pulled..."
    echo "Starting Redis..."
    redis=$(docker -H unix:///docker.sock run -i -t -d -p ${DST_IP}6379:6379 --name shipyard_redis fmyzjs/docker-shipyard-redis)
    sleep 2
    echo "Starting App Router..."
    router=$(docker -H unix:///docker.sock run -i -t -d -p ${DST_IP}80 --link shipyard_redis:redis --name shipyard_router fmyzjs/docker-shipyard-router)
    sleep 2
    echo "Starting Load Balancer..."
    lb=$(docker -H unix:///docker.sock run -i -t -d -p ${DST_IP}80:80 --link shipyard_redis:redis --link shipyard_router:app_router --name shipyard_lb fmyzjs/docker-shipyard-lb)
    sleep 2
    echo "Starting DB..."
    EXTRA_DB_ARGS=""
    ...
    db=$(docker -H unix:///docker.sock run -i -t -d -p ${DST_IP}5432 -e DB_NAME=shipyard -e DB_USER=shipyard -e DB_PASS=$DB_PASS $EXTRA_DB_ARGS --name shipyard_db fmyzjs/docker-shipyard-db)
    sleep 5
...
"
    ...
    docker -H unix:///docker.sock pull fmyzjs/shipyard
